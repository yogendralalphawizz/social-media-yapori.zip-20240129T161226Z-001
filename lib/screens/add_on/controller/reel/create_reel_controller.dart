...
class CreateReelController extends GetxController {
  ...
  //   }
  // }
  void createReel(File? selectedAudioFile, RxList<Media> videoFile, int? competitionId, clubId) async {
    final directory = await getTemporaryDirectory();
    var finalFile = File(
        '${directory.path}/REEL_${DateTime.now().millisecondsSinceEpoch}.mp4');
    ...
    if (selectedAudioFile != null) {
      ...
      var command =
          "-i ${videoFile[0].file!.path} -i ${selectedAudioFile.path} -map 0:v -map 1:a -c:v copy "
          "-shortest ${finalFile.path}";
      FFmpegKit.executeAsync(
        command,
            (session) async {
   ...
   //          Navigator.push(context, route);
      ...
      debugPrint('Reel Created without audio:: ${videoFile[0].file!.path}');
      var finalFile = File(videoFile[0].file!.path);
      // AppUtil.showToast(
      //     context: Get.context!,
      //     message: 'Reel Created without Audio',
      //     isSuccess: true);
