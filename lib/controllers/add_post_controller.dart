      ...
      if (value) {
        ...
        if (media.mediaType == GalleryMediaType.photo) {
          ...
          await File('${tempDir.path}/${media.id!.replaceAll('/', '')}.mp4')
              .create();
          File videoFile = File(media.file!.path);
          Uint8List videoBytes = videoFile.readAsBytesSync();
          file.writeAsBytesSync(videoBytes);
           await ApiController()
              .uploadPostMedia(file.path.toString())
              .then((response) async {

               if(response.success) {
                 String videoPath = response.postedMediaFileName!;
                 await file.delete();
                 gallery = {
                   'filename': videoPath,
                   'video_thumb': videoThumbnailPath.toString() ?? '',
                   'type': competitionId == null ? '1' : '2',
                   'media_type': media.mediaType == GalleryMediaType.photo
                       ? '1'
                       : '2',
                   'is_default': '1',
                 };
                 print('this is video ------>>>>>$gallery');
               }else{
                 isPosting.value = true;
                 update();
               }
          ...
          // await ApiController()
          //     .uploadFile(videoThumbnail.path)
          //     .then((response) async {
          //   videoThumbnailPath = response.postedMediaFileName!;
          //   await videoThumbnail.delete();
          // });
         ...
         Get.offAll(() => const DashboardScreen());
          ...
          ApiController()
            ...
            if (response.post != null) {
              print('ac2');
              print(response.post!.title);
              print(response.post!.user.country);
