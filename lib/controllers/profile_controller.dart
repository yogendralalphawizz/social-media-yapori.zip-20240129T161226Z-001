...
class ProfileController extends GetxController {
  ...
  changePublicPrivateMode(bool status) async {
    if(status == true) {
      // await setDarkMode(status);
      updateProfileMode(paypalId: '2');
      user.value!.isPrivate = true;
    }else{
      updateProfileMode(paypalId: '1');
      user.value!.isPrivate = false;
    }
  }
  ...
  updateBioData({required String paypalId, required BuildContext context}) {
    if (FormValidator().isTextEmpty(paypalId)) {
      AppUtil.showToast(
          message: LocalizationString.pleaseEnterBioData, isSuccess: false);
    } else {
      AppUtil.checkInternet().then((value) {
        if (value) {
          EasyLoading.show(status: LocalizationString.loading);
          ApiController().updateBio(paypalId).then((response) async{
            if (response.success == true) {
              EasyLoading.dismiss();
              Future.delayed(const Duration(milliseconds: 800), () {
                Navigator.pop(context);
              });
          await  AppUtil.showToast(
                  message: LocalizationString.bioDataUpdated,
                  isSuccess: true);
              _userProfileManager.refreshProfile();
            // Fluttertoast.showToast(msg: LocalizationString.bioDataUpdated);
  ...
  updateQualification({required String qualification, required BuildContext context}) {
    if (FormValidator().isTextEmpty(qualification)) {
      AppUtil.showToast(
          message: LocalizationString.pleaseEnterQualification, isSuccess: false);
    } else {
      AppUtil.checkInternet().then((value) {
        if (value) {
          EasyLoading.show(status: LocalizationString.loading);
          ApiController().updateQualification(qualification).then((response) async{
            if (response.success == true) {
              EasyLoading.dismiss();
              Future.delayed(const Duration(milliseconds: 800), () {
                Navigator.pop(context);
              ...
              await  AppUtil.showToast(
                  message: LocalizationString.qualificationUpdated,
                  isSuccess: true);
              _userProfileManager.refreshProfile();
              // AppUtil.showToast(
              //     message: LocalizationString.qualificationUpdated,
  ...
  updateGender({required String sex, required BuildContext context}) {
    if (FormValidator().isTextEmpty(sex)) {
      AppUtil.showToast(
          message: LocalizationString.pleaseEnterGender, isSuccess: false);
    } else {
      AppUtil.checkInternet().then((value) {
        if (value) {
          EasyLoading.show(status: LocalizationString.loading);
          ApiController().updateGender(sex).then((response) {
            if (response.success == true) {
              EasyLoading.dismiss();
              AppUtil.showToast(
                  message: LocalizationString.genderUpdated,
                  isSuccess: true);
              _userProfileManager.refreshProfile();
  ...
  updateWebsiteLinks({required String websites, required BuildContext context}) {
    if (FormValidator().isTextEmpty(websites)) {
      AppUtil.showToast(
          message: LocalizationString.pleaseEnterWebsite, isSuccess: false);
    } else {
      AppUtil.checkInternet().then((value) {
        if (value) {
          EasyLoading.show(status: LocalizationString.loading);
          ApiController().updateWebsites(websites).then((response) {
            if (response.success == true) {
              EasyLoading.dismiss();
              AppUtil.showToast(
                  message: LocalizationString.websiteUpdated,
                  isSuccess: true);
              _userProfileManager.refreshProfile();
              Future.delayed(const Duration(milliseconds: 800), () {
                _userProfileManager.refreshProfile();
  ...
  updateProfileMode({required String paypalId, }) {
    if (FormValidator().isTextEmpty(paypalId)) {
      // AppUtil.showToast(
      //     message: LocalizationString.pleaseEnterBioData, isSuccess: false);
    } else {
      AppUtil.checkInternet().then((value) {
        if (value) {
          EasyLoading.show(status: LocalizationString.loading);
          ApiController().privateModeApi(paypalId).then((response) {
            if (response.success == true) {
              EasyLoading.dismiss();
              AppUtil.showToast(
                  message: LocalizationString.profileVisibilityChanged,
                  isSuccess: true);
              _userProfileManager.refreshProfile();
          ...
          ApiController().updateUserName(userName).then((response) {
            if (response.success == true) {
              ...
              AppUtil.showToast(
                  ...
                  isSuccess: true);
              updateLocation(country: country, city: 'city', context: context, isSignup: true);
              getMyProfile();
              if (isSigningUp == true) {
      ...
      if (value) {
        ...
        ApiController()
          ...
          if (response.success == true) {
            EasyLoading.dismiss();
            AppUtil.showToast(
                message: LocalizationString.categoryTypeUpdated,
                isSuccess: true);
            getMyProfile();
            if (isSigningUp == true) {
